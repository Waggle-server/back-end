...
function chat_listR(parameter) {
    return new Promise((resolve, reject) => {
        const queryData = `SELECT room_key, chat_list.title, nickname, img, chat_list.post_key, chat_list.type FROM chat_list 
...
function chat_companion_R(parameter) {
    return new Promise((resolve, reject) => {
        const queryData = `SELECT chat_key, chating.user_key as user_key, msg, date, nickname, img FROM chating LEFT OUTER JOIN user ON chating.user_key = user.user_key where chat_key = ?`;
        db.query(queryData, [parameter], (err, db_data) => {
            if(db_data) resolve(db_data);
            else reject(err);
        })
